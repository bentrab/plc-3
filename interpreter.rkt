...
; evaluates a funtion-call and returns its return value
(define interpret-function-call
  (lambda (function-call environment throw)
    (call/cc
     (lambda (func-return)
       (cond
         ((not (exists? (funcname function-call) environment)) (myerror "Function does not exist")) ;checks if the function exists
         ((null? (params function-call)) (interpret-function-statement-list (statement-list-of-function (lookup (funcname function-call) environment)) (push-frame (pop-frame environment)) func-return breakOutsideLoopError continueOutsideLoopError throw)) ; checks if there are parameters
         (else (interpret-function-statement-list (statement-list-of-function (lookup (funcname function-call) environment)) (add-parameters-to-environment (func-name (lookup (funcname function-call) environment)) (params function-call) (push-frame environment) throw) func-return breakOutsideLoopError continueOutsideLoopError throw)))))))

; evealuates a function call and returns its associated environment
(define interpret-function-call-result-environment
 (lambda (statement-list environment return break continue throw)
  (cond
    ...
    (else (if (list? (call/cc
                  ...
                  (interpret-function-call-result-environment (cdr statement-list) (call/cc
                                                                              (lambda (breakreturn)
                                                                                (interpret-statement (car statement-list) environment breakreturn break continue throw)))
                                                        return break continue throw)
                  environment)))))
...
(define add-parameters-to-environment
  (lambda (param-names param-values environment throw)
    (cond
      ...
      ((list? param-names) (add-parameters-to-environment (params param-names) (params param-values) (insert (car param-names) (eval-expression (car param-values) (pop-frame environment) throw) environment) throw))
